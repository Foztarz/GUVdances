...
formula_all = bf(#modulus may not be necessary, included in lpd function
  ...
  mu0 ~ 1 + BR + CL + BR:CL, #population level effects
  ...
  kappa ~ 1 + BR + CL + BR:CL + (1 + BR + CL + BR:CL | ID), #concentration for primary mean
...
#priors for mu
pr_mu_all = prior('normal(0, pi()/1)', coef = 'Intercept', nlpar = 'mu0') +
...
for(i in 1:20 )
{
  ...
  mubc_name = paste0('b_muibc_BC:ID',u_id[i])
  ...
  kappabc_name = paste0('draws_all$`r_ID__kappa[',u_id[i],',BRl:CLu]`')
  ...
  Draws2Cont(draws_all,
             x_string = 'sin(b_mu0_Intercept + get(mu_name) + 
                                   b_mu0_BRl + b_mu0_CLu + draws_all$`b_mu0_BRl:Clu` +
                                   get(mub_name) + get(muc_name) + get(mubc_name))*
                         A1(softplus(Intercept_kappa +get(kappa_name)+
                                   b_kappa_BRl + b_kappa_CLu + draws_all$`b_kappa_BRl:CLu` +
