...
pr_mu_all = prior('normal(0, pi()/6)', coef = 'Intercept', nlpar = 'mu0') +
                  ...
                  prior('normal(-pi()/2, pi()/12)', class = 'b', coef = 'BC', nlpar = 'mu0') +
            ...
            set_prior("target += lognormal_lpdf(log1p_exp(softkappamu) | log(30), 0.6)", #expect high concentration (low variation) 
                      check = FALSE)  +
  # prior('unwrap_von_mises_vect(0, log1p_exp(16))',
  #                       nlpar  = 'muib',  class = 'b')+
  prior('unwrap_von_mises_vect(0, log1p_exp(softkappamu + softkappamub))',
                        nlpar  = 'muib',  class = 'b') +
            set_prior("target += normal_lpdf(softkappamub | 0, 0.2)", #expect small differences 
                      check = FALSE)  +
  # prior('unwrap_von_mises_vect(0, log1p_exp(30))',
  #                       nlpar  = 'muic',  class = 'b')+
  prior('unwrap_von_mises_vect(0, log1p_exp(softkappamu + softkappamuc))',
                        nlpar  = 'muic',  class = 'b') +
            set_prior("target += normal_lpdf(softkappamuc | 0, 0.2)", #expect small differences 
                      check = FALSE) +
  # prior('unwrap_von_mises_vect(0, log1p_exp(20))',
  #                       nlpar  = 'muibc',  class = 'b') +
  prior('unwrap_von_mises_vect(0, log1p_exp(softkappamu + softkappamubc))',
                        nlpar  = 'muibc',  class = 'b') +
            set_prior("target += normal_lpdf(softkappamubc | 0, 0.5)", #expect small differences 
                      check = FALSE)  
...
pr_kappa_all = 
                  ...
                  prior('student_t(3, 0, 0.3)', class = 'sd', dpar = 'kappa')
                  prior('student_t(3, 0, 0.2)', class = 'sd', coef = 'B', dpar = 'kappa')+
                  prior('student_t(3, 0, 0.2)', class = 'sd', coef = 'C', dpar = 'kappa')+
                  prior('student_t(3, 0, 0.2)', class = 'sd', coef = 'BC', dpar = 'kappa')
...
system.time(
  {
    ...
    model_all = brm(formula = formula_all,
                   # data = subset(cd_all, ID %in% u_id[20+1:50]), #target some balanced cases
                   data = cd_all,
                   prior = pr_all,
                   ...
                   cores = 4, # on 4 CPUs
                   warmup = 1000, #
                   iter = 1500, # longer sampling to efficiently sample mu0
                   refresh = 500,
