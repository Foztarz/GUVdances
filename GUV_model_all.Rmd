...
# To Do
- Reparametrise kappamu to Bambi style
- Achieve mu0 convergence with very informative prior
- Extract individual differences in heading +
  - Separate individual calculations from plotting +
- Plot model predictions +
- Plot model contrasts +
- Fit bimodality to contrasts
- Make plot
- Make hmtl
...
real softkappamuc;
                           ...
                           block = "parameters") + 
  stanvar(scode = "
real kappa_muc = log1p_exp(softkappamuc);
          ", 
          block = 'genquant')
...
real softkappamubc;
                           ...
                           block = "parameters") + 
  stanvar(scode = "
real kappa_mubc = log1p_exp(softkappamubc);
          ", 
          block = 'genquant')
...
mu0_Intercept N(0,pi/2) -> N(0,pi/6) : most chains around 15° but drift, tighten
b_kappa (flat) -> N(0,1) : avoid extremely low kappa for conditions!
b=sd_kappa T(3,0,1.0) -> T(3,0,0.5) : avoid extremely low kappa for conditions!
mu0_Intercept N(0,pi/6) -> N(0,pi/12) : worse mu0, weaken softkappamu prior
mu0_Intercept N(0,pi/12) -> N(0,pi/2)
softkappamu LN(log(30), 0.6) -> LN(log(10), 0.6)
mu0_Intercept N(0,pi/2) -> N(0,pi/4)
sd_kappa T(3,0,1.0) -> T(3,0,0.5) : avoid extremely low kappa for conditions
sd_kappa T(3,0,1.0) -> T(3,0,1.0) : to allow some gradient
b sd_kappa T(3,0,0.5) -> T(3,0,0.5)
b sd_kappa T(3,0,0.5) -> T(3,0,0.7) : to allow some gradient
b sd_kappa  T(3,0,0.7) -> T(3,0,0.3) 
Intercept sd_kappa  T(3,0,1.0) -> T(3,0,0.5) 
Intercept_kappa N(3,3) -> N(3,1) 

Very restrictive priors from GUV_unwrap.ipynb (but diff kappamu format)
Very restrictive priors from GUV_unwrap.ipynb (updated to account for UV dim 
Very restrictive priors from GUV_unwrap.ipynb with BC left turn
Very restrictive priors from GUV_unwrap.ipynb (and _same_ kappamu format)
```{r Unimodal change priors}

#priors for mu
pr_mu_all = prior('normal(0, pi()/6)', coef = 'Intercept', nlpar = 'mu0') +
                  prior('normal(0, pi()/12)', class = 'b', nlpar = 'mu0') +
                  prior('normal(0, pi()/12)', class = 'b', coef = 'BC', nlpar = 'mu0') +
  ...
  prior('unwrap_von_mises_vect(0, log1p_exp(softkappamu))',
                        nlpar  = 'muib',  class = 'b') +
            set_prior("target += lognormal_lpdf(log1p_exp(softkappamub) | log(30), 0.1)", #expect small differences 
                      check = FALSE)  +
  prior('unwrap_von_mises_vect(0, log1p_exp(softkappamuc))',
                        nlpar  = 'muic',  class = 'b') +
            set_prior("target += lognormal_lpdf(log1p_exp(softkappamuc) | log(30), 0.1)", #expect small differences 
                      check = FALSE) +
  prior('unwrap_von_mises_vect(0, log1p_exp( softkappamubc))',
                        nlpar  = 'muibc',  class = 'b') +
            set_prior("target += lognormal_lpdf(log1p_exp(softkappamubc) | log(30), 0.1)", #expect small differences 
                      check = FALSE)  

#priors for kappa
pr_kappa_all = 
                  prior('normal(3,0.5)', class = 'Intercept', dpar = 'kappa') +
                  prior('normal(0,0.3)', class = 'b', dpar = 'kappa')+
                  prior('normal(0,1.0)', class = 'b', coef = 'BC', dpar = 'kappa')+
                  prior('student_t(3, 0, 0.5)', class = 'sd', dpar = 'kappa')
                  prior('student_t(3, 0, 0.5)', class = 'sd', coef = 'B', dpar = 'kappa')+
                  prior('student_t(3, 0, 0.5)', class = 'sd', coef = 'C', dpar = 'kappa')+
                  prior('student_t(3, 0, 0.5)', class = 'sd', coef = 'BC', dpar = 'kappa')
...
For all individuals, expect this to take around 12 hours!
...
system.time(
  {
    ...
    model_all = brm(formula = formula_all,
                   # data = subset(cd_all, ID %in% u_id[20+1:20]), #target some balanced cases
                   data = cd_all,
                   prior = pr_all,
                   ...
                   cores = 4, # on 4 CPUs
                   warmup = 1000, #
                   iter = 2000, # longer sampling to efficiently sample mu0
                   refresh = 250,
...
plot(model_all,
      ask = FALSE,
      variable = c('b_kappa_Intercept','b_kappa_B', 'b_kappa_C', 'kappa_mu', 'sd_ID__kappa'),
      nvariables = 4,
