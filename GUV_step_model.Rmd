...
formula_symm = bf(#modulus may not be necessary, included in lpd function
  formula = change_angle ~ mu,
  nlf( mu1 ~ -(pi()*inv_logit(logitmu0) + mui) ), #primary mean, population mean plus individual mean
  nlf( mu2 ~ pi()*inv_logit(logitmu0)  + mui ), #secondary mean, population mean plus offset, plus individual mean
  logitmu0 ~ 1, #primary mean
  mui ~ 0 + ID, # mean angle combines fixed and random effects
  nlf( kappa1 ~ k1 ), #concentration for primary mean
  nlf( kappa2 ~ k1 ), #concentration for secondary mean
  k1 ~ 1 + (1 |ID), #for kappa this occurs in linear space, and BRMS can set it up automatically
...
#priors for mu
pr_mu_symm = #prior('normal(-pi()/2, pi()/4)', class = 'b', nlpar = 'mu0') +
...
plot(model_symm,
      ask = FALSE,
      variable = c('k1_Intercept', 'theta1_Intercept', 'sd', 'kappa_mu'),
      regex = TRUE) #plot raw estimates
#population directional bias
# plot(model_symm,
#       ask = FALSE,
...
plot(model_symm,
      ...
      variable = 'mu0',
      regex = TRUE,
     transform = plogis) #plot transformed estimates
#population level change in direction
...
for(i in 1:20)#length(id_uv) )
{
  ...
  Draws2Cont(draws_symm,
             x_string = 'sin(-(pi*plogis(b_logitmu0_Intercept) + get(mu_name)))*
                         A1(softplus(b_k1_Intercept+get(kappa_name)))',
             y_string = 'cos(-(pi*plogis(b_logitmu0_Intercept) + get(mu_name)))*
                         A1(softplus(b_k1_Intercept+get(kappa_name)))',
             palette = 'Red-Yellow' 
             )
  #secondary mean
  ...
  mu1_id_symm[i] = with(draws_symm,
                      {
                        median.circular(
                             circular(x =
                             mod_circular( -(pi*plogis(b_logitmu0_Intercept) + get(mu_name)) ),
                                          template = 'none')
                                           )[1]
                      })
...
Draws2Cont(draws = draws_symm,
           x_string = 'sin(-pi*plogis(b_logitmu0_Intercept))*
             A1(kappa_mu)',
           y_string = 'cos(-pi*plogis(b_logitmu0_Intercept))*
             A1(kappa_mu)',
           palette = 'Red-Yellow'
           )
Draws2Cont(draws = draws_symm,
...
with(draws_symm,
     arrows.circular(x = mean.circular(circular(-pi*plogis(b_logitmu0_Intercept),
                                                  units = 'radians',
                                                  rotation = 'clock',
                                                  zero = pi/2)
     )[1],
