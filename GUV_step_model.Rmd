...
formula_devchange = bf(#modulus may not be necessary, included in lpd function
  ...
  nlf(mu2 ~ modulo_circular(mu0 + pi() + mui + mubi) ), #secondary mean, population mean plus offset, plus individual mean, plus additional individual offset
  mu0 ~ 1, #primary mean
  mui ~ 0 + ID, # mean angle combines fixed and random effects
  # mub ~ 1, #angle between primary mean and secondary mean
  mubi ~ 0 + ID, # individual devations
  nlf(kappa1 ~ k1), #concentration for primary mean
  nlf(kappa2 ~ k1), #concentration for secondary mean
  k1 ~ 1 + (1 |ID), #for kappa this occurs in linear space, and BRMS can set it up automatically
  theta1 ~ 1 + (1 |ID), #for mixture weighting, this is specific to individual and condition combination
...
#priors for mu
pr_mu_devchange = prior('normal(-pi()/2, pi()/3)', class = 'b', nlpar = 'mu0') +
...
#priors for theta
pr_theta_devchange = 
                  prior('normal(4.5, 0.5)', class = 'Intercept', dpar = 'theta1') +
...
for(i in 1:20)#length(id_uv) )
{
  ...
  #secondary mean
  Draws2Cont(draws_devchange,
             x_string = 'sin(b_mu0_Intercept + b_mub_Intercept + get(mu_name) + get(mub_name))*
                         A1(softplus(b_k1_Intercept+get(kappa_name)))',
             y_string = 'cos(b_mu0_Intercept + b_mub_Intercept + get(mu_name) + get(mub_name))*
                         A1(softplus(b_k1_Intercept+get(kappa_name)))',
             palette = 'Blue-Yellow' 
             )
  ...
  mu2_id_devchange[i] = with(draws_devchange,
                      {
                        median.circular(
                             circular(x =
                             mod_circular(b_mu0_Intercept + b_mub_Intercept + get(mu_name) + get(mub_name)),
                                          template = 'none')
                                           )[1]
                      })
