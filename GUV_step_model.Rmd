...
sm_symm = summary(model_symm,
                 variable = c('Intercept_kappa', 'sd', 'kappa_mu'),
                regex = TRUE)
print(sm_symm$fixed[c(1:2,3+length(id_uv)),], digits = 3)
...
formula_devchange = bf(#modulus may not be necessary, included in lpd function
  ...
  nlf(mu2 ~ -(modulo_circular(mu0) + mui2) ), #secondary mean, population mean plus offset, individual mean, plus additional individual offset
  mu0 ~ 1, #primary mean
...
sm_devchange = summary(model_devchange,
                 variable = c('Intercept_kappa', 'theta1_Intercept',
                              'sd', 'kappa_mu'),
                regex = TRUE)
print(sm_devchange$fixed[c(1:2,3+2*length(id_uv)),], digits = 3)
...
{
  mu_name = paste0('b_mui_ID',id_uv[i])
  mub_name = paste0('b_mubi_ID',id_uv[i])
  kappa_name = paste0('r_ID__k1[',id_uv[i],',Intercept]')
  # theta_name = paste0('r_ID__theta1[',id_uv[i],',Intercept]')
  with(subset(cd_change, ID %in% id_uv[i]),
  {
  ...
  #secondary mean
  Draws2Cont(draws_devchange,
             x_string = 'sin( -(mod_circular(b_mu0_Intercept) + get(mub_name)) )*
                         A1(softplus(b_k1_Intercept+get(kappa_name)))',
             y_string = 'cos( -(mod_circular(b_mu0_Intercept) + get(mub_name)) )*
                         A1(softplus(b_k1_Intercept+get(kappa_name)))',
             palette = 'Blue-Yellow' 
             )
  ...
  mu2_id_devchange[i] = with(draws_devchange,
                      {
                        MedCDraws(
                             -(mod_circular(b_mu0_Intercept) + get(mub_name) )
                                           )
                      })
