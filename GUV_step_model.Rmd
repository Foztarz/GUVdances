...
formula_change = bf(#modulus may not be necessary, included in lpd function
  ...
  nlf(mu2 ~ modulo_circular(mu0 + pi() + mui)), #secondary mean, population mean plus offset, plus individual mean, plus additional individual offset
...
#priors for theta
pr_theta_change = 
                  prior('normal(3.0, 0.25)', class = 'Intercept', dpar = 'theta1') +
...
for(i in 1:20)#length(id_uv) )
{
  ...
  #            )
  Draws2Cont(draws_change,
             x_string = 'sin(b_mu0_Intercept + pi + get(mu_name))*
                         A1(softplus(b_k1_Intercept+get(kappa_name)))',
             y_string = 'cos(b_mu0_Intercept + pi + get(mu_name))*
                         A1(softplus(b_k1_Intercept+get(kappa_name)))',
             palette = 'Blue-Yellow' 
             )
  ...
  mu2_id_change[i] = with(draws_change,
                      {
                        median.circular(
                             circular(x =
                             mod_circular(b_mu0_Intercept + pi + get(mu_name)),
                                          template = 'none')
                                           )[1]
                      })
...
#            )
Draws2Cont(draws = draws_change,
           x_string = 'sin(b_mu0_Intercept+pi)*
             A1(kappa_mu)',
           y_string = 'cos(b_mu0_Intercept+pi)*
             A1(kappa_mu)',
           palette = 'Blue-Yellow'
           )
...
formula_devchange = bf(#modulus may not be necessary, included in lpd function
  formula = change_angle ~ mu,
  nlf(mu1 ~ modulo_circular(mu0 + mui) ), #primary mean, population mean plus individual mean
  nlf(mu2 ~ modulo_circular(mu0 + mub + mui + mubi) ), #secondary mean, population mean plus offset, plus individual mean, plus additional individual offset
  mu0 ~ 1, #primary mean
  mui ~ 0 + ID, # mean angle combines fixed and random effects
  mub ~ 1, #angle between primary mean and secondary mean
