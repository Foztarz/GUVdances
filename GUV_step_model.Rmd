...
formula_change = bf(#modulus may not be necessary, included in lpd function
  ...
  nlf(mu2 ~ mu0 + mub + mui + mubi), #secondary mean, population mean plus offset, plus individual mean, plus additional individual offset
  mu0 ~ 1, #primary mean
  mui ~ 0 + ID, # mean angle combines fixed and random effects
...
#priors for mu
pr_mu_change = prior('normal(-pi()/2, pi()/4)', class = 'b', nlpar = 'mu0') +
...
plot(model_symm,
      ask = FALSE,
      variable = c('Intercept_kappa', 'theta1_Intercept', 'k1_Intercept', 'sd', 'kappa_mu'),
      regex = TRUE) #plot raw estimates
#population directional bias
# plot(model_symm,
#       ask = FALSE,
