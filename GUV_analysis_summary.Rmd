...
rspar = do.call(rbind, c(ml_same_lst, ml_diff_lst))
rownames(rspar) = paste( sort(
                          rep(x = c( 'same', 'diff'),
...
rspar = rspar[,c('mu', 'kappa', 'se.mu', 'se.kappa')]
...
mle_CI = with(data.frame(do.call(rbind, c(ml_same_lst, ml_diff_lst))), 
              {
              mapply(FUN = CI_vM, 
...
print(unlist(mod_CI[3:4]), digits = 3) #find a better display method
