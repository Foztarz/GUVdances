...
```{r Plot model predictions, echo = FALSE}
...
contr_phi = with(data.frame(draws_phi),
                {
                  data.frame(
                    #bright green -> dim green = low brightness effect
                    gh_gl_phi = b_phi_brightnl, 
                    #bright UV -> dim UV = dim + UVdim - UV effect
                    uvh_uvl_phi = b_phi_brightnl + `b_phi_brightnl.colouru` - b_phi_colouru, 
                    #bright green -> bright UV = UV effect
                    gh_uvh_phi = b_phi_colouru, 
                    #bright green -> dim UV = UV + dim + UVdim
                    gh_uvl_phi = b_phi_brightnl + b_phi_colouru + b_phi_brightnl.colouru, 
                    #dim green -> dim UV = UV effect + UVdim - dim effect
                    gl_uvl_phi = b_phi_colouru + `b_phi_brightnl.colouru` - b_phi_brightnl
                  )
                }
                )
...
stripchart(x = contr_all,
           at = 1:dim(contr_all)[2],
           group.names = gsub(pattern = '_',
                              replacement = ' - ',
                              x = names(contr_all)),
           main = 'Beta GLMM - Contrasting Effects',
           vertical  = TRUE,
           method = 'jitter',
           offset =  0.5,
           ylim = c(-1, 1)*6,
...
#                             ), alpha = 0.5),
#         outline = FALSE,
#         range = 0, #extend to extremes
#         pars = list(boxwex = 0.5,
#                     staplewex = 0),
#         axes = FALSE
#            )
with(data.frame(t(quant_all)),
     {
      arrows(x0 = 1:dim(contr_all)[2],
             y0 = X2.5.,
             y1 = X97.5.,
             length = 0, 
             angle = 90,
