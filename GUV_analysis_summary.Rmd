...
```
We can successfully fit an aligned ranks model to compare across individuals. Note that the "boundary" level random effects indicate that individual effects were too small to estimate. In the individual linear models (e.g. `summary(artlm(mv_full_art, "brightn"))$varcor`) there may still be substantial individual differences. The primary effect appears to be that of brightness, but there may also be effects of interest in the post-hoc contrasts.
...
```
The post-hoc tests indicate a large difference between bright and dim conditions, as expected, but very little evidence for differences between colours. The interactions (for which there was low support in the overall model) reflect the pattern we expect, that all bright conditions have higher accuracy than all dim conditions, and that while the two dim conditions have similar accuracy, the bright UV bright condition may be slightly more accurate the bright green condition (p = 0.158).
### Parametric beta-distributed model of accuracy
To investigate this with greater statistical power we would need an appropriate parametric model for bounded variables. One approach is to use the beta distribution, which is defined between (but not at) 0 and 1.
