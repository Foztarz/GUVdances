...
#TODO parallelise and move to functions
lambda_uhl = replicate(n = 1e3,
               expr = 
                 {
               pr = 
                    M5B(data = circular(
                                        rad(
                                          sample(unlist(mu_diff_uhl),
                                               replace = TRUE,
                                               size = length(mu_diff_uh))
                                          ),
                                        template = 'none',
                                        zero = 0),
                        niter = 1000
          ...
          #check which is to the right and return its weight
          return(
            if(pr[1]<pr[3])
              {1-pr[5]}else
              {pr[5]}
...
hist(lambda_uhl, breaks = 20, xlim = c(0,1), border = NA)
quantile(lambda_uhl,
