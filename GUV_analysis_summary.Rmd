...
> -   move raw data to end
> -   make functions for result extraction from BRMS
> -   neaten up priors for BRMS models
...
with(subset(cd, ID %in% ii & run %in% c(10, 20, 40)),
     {
...
### Parametric beta-distributed model of accuracy To investigate this with greater statistical power we would need an appropriate parametric model for bounded variables.
...
#TODO remove unnecessary priors
...
```{r Fit the Bayesian beta model, echo = FALSE}
## Run the models --------------------------------------------------------
...
md_beta = brm(formula = frm_bmod_maxRE,
                 ...
                 data = within(mean_vectors_full,
                               {
                               mean_vector = ifelse(mean_vector == 1.0,
                                                    yes = 1.0 - 1e-3,
                                                    no = mean_vector)
                               }
                               ),
...
```{r Plot accuracy model predictions, echo = FALSE}
...
stripchart(x = round(contr_mu,2),
           at = 1:dim(contr_mu)[2],
           group.names = gsub(pattern = '_',
                              replacement = ' → ',
                              x = names(contr_mu)),
           main = 'Beta GLMM - Effects on Mean Accuracy',
           ylab = 'logit mean change in accuracy',
           vertical  = TRUE,
           method = 'stack',
           offset =  0.05,
           ylim = c(-1, 1)*4,
           pch = 20,
           col = gray(0, alpha = 4/2e3),
           cex.axis = 1.0,
           las = 3
           )
abline(h = 0,
       lty = 2)
with(data.frame(t(quant_mu)),
     {
      arrows(x0 = 1:dim(quant_mu)[2],
             y0 = X2.5.,
             y1 = X97.5.,
             length = 0, 
             angle = 90,
...
stripchart(x = round(contr_phi,2),
           at = 1:dim(contr_phi)[2],
           group.names = sub(pattern = '_',
                              replacement = ' → ',
                              x = names(contr_phi)),
           main = 'Effects on Accuracy Precision',
           ylab = 'log change in accuracy precision',
           vertical  = TRUE,
           method = 'stack',
           offset =  0.05,
           ylim = c(-1, 1)*6,
...
abline(h = 0,
       lty = 2)
with(data.frame(t(quant_phi)),
     {
      arrows(x0 = 1:dim(quant_phi)[2],
             y0 = X2.5.,
             y1 = X97.5.,
             length = 0, 
             angle = 90,
...
beta_all_res = data.frame(mu_lower = quant_mu[1,],
...
```{r Fit the sun elevation model, echo = FALSE}
## Run the models --------------------------------------------------------
system.time( #takes about 2 minutes
  {
...
```{r Plot sun elevation model predictions, echo = FALSE}
# Extract model predictions -----------------------------------------------
...
In that data, there was a clear sigmoidal relationship between intensity and accuracy (mean vector length). They mention that the particular relationship with light intensity was variable, but that the relative relationships remained constant, so we may also observe the same relationship but shifted.
...
require(geosphere)#TODO move to functions
# Define the coordinates

hive_coords  =  c(longitude  = 8.811333,
                 latitude = 50.806306)
# garden_coords = c(longitude  = 8.806694,
#                   latitude = 50.800917) # flower beds
#centre according to google 50.802605, 8.809097
# left according to openstreetmap 50.802721, 8.806807



garden_coords = list(left = 
                       c(longitude  = 8.806807,
                         latitude = 50.802721),
                     centre = 
                       c(longitude  = 8.809097,
                         latitude = 50.802605),
                     right = 
                       c(longitude  = 8.811926,
                         latitude = 50.803182)
                     )
...
#TODO save and load models instead of running them each time
load_model = TRUE
# Custom family --------------------------------------------------------
...
#TODO oops I forgot this is a huge amount of data, I'll try the full set later

# dt_photo = subset(dt_photo,
#                   subset = ID %in% full_ids)
...
formula_photo = bf(
  formula = angle ~ mu,
            nlf(mu ~ fmu + zmu), 
            fmu ~ 0, # fixed effects intercept == 0
            zmu ~ 0 + ID,
            kappa ~ 1 + (1|ID),
...
formula_nophoto = bf(
  formula = angle ~ mu,
            nlf(mu ~ fmu + zmu), 
            fmu ~ 1, # fixed effects intercept estimated
            zmu ~ 0 + ID,
            kappa ~ 1 + (1|ID),
  family = unwrap_von_mises,#mod mu, kappa via the softplus
  nl = TRUE)#to accept user-defined extra parameters (zmu) we need to treat the formula as nonlinear
...
formula_sunspot = bf(
  formula = bgangle ~ mu,
            nlf(mu ~ fmu + zmu), 
            fmu ~ 0, # fixed effects intercept estimated
            zmu ~ 0 + ID,
            kappa ~ 1 + (1|ID),
  family = unwrap_von_mises,#mod mu, kappa via the softplus
  nl = TRUE)#to accept user-defined extra parameters (zmu) we need to treat the formula as nonlinear
...
pr_photo = 
  set_prior(paste("target +=", 
                  'unwrap_von_mises_vect_lpdf(b_zmu | 0, log1p_exp(kappamu))',
                  '+ normal_lpdf(b_zmu | 0, 2*pi())'# additional prior to keep estimates from walking around the circle
  ),
  check = FALSE) +
  set_prior("target += normal_lpdf(kappamu | 1.0, 3.0)", #prior to lower values, indiv differences should be large
            check = FALSE) +
...
pr_nophoto = pr_photo +
  prior(normal(0,2*pi()), class = b,  nlpar = 'fmu', coef = 'Intercept')
...
if(load_model)
{ load(file = '1Data/phototaxis_modlist.RData')
}else
  {
...
lapply(X = modlist[c(2,4)],
       FUN = UnwrapRhats,
       variable = '^b_fmu')
#TODO check what is wrong with nosunspot
lapply(X = modlist,
       FUN = SumUwRhats)
...
save(modlist,
     file = '1Data/phototaxis_modlist.RData')
}
...
# bayestestR::rope(x = modlist$no_phototaxis,
#                  range = c(-1,1)*pi/4,
#                  parameters = 'fixed_fmu',
#                  effects = 'all',
#                  verbose = FALSE)
# bayestestR::rope(x = modlist$no_sunspot,
#                  range = c(-1,1)*pi/4,
#                  parameters = 'fmu_Intercept',
#                  effects = 'all',
#                  verbose = FALSE)
dr_modlist = lapply(modlist,
                    as_draws_df)
#for each individual, calculate the likelihood of foraging in the expected region

PostROPE = function(id, rope, drs, mu = 'b_fmu_Intercept')
{
...
abline(h = 0)
```
It is hard to say for the individuals whether the majority  orient towards the sector containing the Botanic Garden, or a similar sized sector containing the stimulus. What we can say is that the population average falls directly towards the Botanic Garden centre in real-world bearings, while the population average relative to the light source has a leftwards bias.

```{r plot sunspot hypothesis tests}
with(dr_modlist$no_phototaxis, paste0(mean(b_fmu_Intercept > 0)*100, '%') ) #nearly all estimates suggest a leftwards bias
with(dr_modlist$no_sunspot, paste0(mean(b_fmu_Intercept > 0)*100, '%') ) #there is notable overlap between the high-density posterior and centre of Botanic Gardens.
...
with(dr_modlist$phototaxis, 
     arrows(x0 = 0, x1 = 0,
            y0 = 0, 
            y1 = #A1(softplus(median(b_kappa_Intercept)))
              A1(softplus(median(kappamu))),
            col = 'orange',
            lwd = 5,
            length = 0.1
            )
)
# Draws2Cont(dr_modlist$no_phototaxis,
#            x_string = paste0('sin(',
#                              'b_fmu_Intercept',
#                              ')*',
#                              'A1(softplus(',
...
with(dr_modlist$no_phototaxis, 
     arrows(x0 = 0,
            x1 = #A1(softplus(median(b_kappa_Intercept)))*
              A1(softplus(median(kappamu)))*
              sin(median(mod_circular(b_fmu_Intercept))),
            y0 = 0, 
            y1 = #A1(softplus(median(b_kappa_Intercept)))*
              A1(softplus(median(kappamu)))*
              cos(median(mod_circular(b_fmu_Intercept))),
            col = 'green3',
            lwd = 5,
            length = 0.1
            )
...
with(as_draws_df(modlist$no_sunspot), 
     arrows(x0 = 0,
            x1 = #A1(softplus(median(b_kappa_Intercept)))*
              A1(softplus(median(kappamu)))*
              sin(median(mod_circular(b_fmu_Intercept+garden_bearing_rad                                         ))),
            y0 = 0, 
            y1 = #A1(softplus(median(b_kappa_Intercept)))*
              A1(softplus(median(kappamu)))*
              cos(median(mod_circular(b_fmu_Intercept+garden_bearing_rad 
                                        ))),
            col = 'green3',
            lwd = 5,
            length = 0.1
...
# Draws2Cont(dr_modlist$no_sunspot,
#            alpha = 100/255,
#            x_string = paste0('sin(',
#                              'b_fmu_Intercept+garden_bearing_rad',
#                              ')*',
#                              'A1(softplus(',
#                              'b_kappa_Intercept',
#                              '))'),
...
with(dr_modlist$no_phototaxis,
     {
      VertHist(data = deg(b_fmu_Intercept), 
       ...
       lwd = 7)
       arrows(x0 = 0, x1 = 0,
              y0 = quantile(deg(b_fmu_Intercept), p = 0.025),
              y1 = quantile(deg(b_fmu_Intercept), p = 1-0.025),
              col = 'green4',
              lwd = 5,
              code = 3,
              length = 0.05,
...
with(dr_modlist$no_sunspot,
     {
      VertHist(data = deg(b_fmu_Intercept)  + gb_pos, 
...
with(sm_modlist$no_sunspot,
     paste0(deg(MardiaSD(spec_pars['kappa_mu' ,'Estimate'])), '°') )
...
wt_uv_hl = all_res['uv_hl', 'weight1']
