...
formula_uni = bf(#modulus may not be necessary, included in lpd function
  formula = angle ~ mu,
  nlf(mu ~ mu0 + mui +muib +muic), #primary mean, population mean plus individual mean
...
For all individuals, expect this to take around 2.5 hours!
...
print(sm_uni$fixed[c(1:2,5:6+length(u_id)),], digits = 3)
...
# for(i in 1:20 )
{
  ...
  mub_name = paste0('b_muib_B:ID',u_id[i])
  muc_name = paste0('b_muic_C:ID',u_id[i])
  kappa_name = paste0('r_ID__kappa[',u_id[i],',Intercept]')
  kappab_name = paste0('r_ID__kappa[',u_id[i],',BRl]')
  kappac_name = paste0('r_ID__kappa[',u_id[i],',CLu]')
  ...
  Draws2Cont(draws_uni,
             x_string = 'sin(b_mu0_Intercept + get(mu_name) + 
                                   b_mu0_CLu + get(muc_name))*
                         A1(softplus(Intercept_kappa+get(kappa_name) +
                                       b_kappa_CLu +get(kappac_name)))',
             y_string = 'cos(b_mu0_Intercept + get(mu_name) + 
                                 b_mu0_CLu + get(muc_name))*
                         A1(softplus(Intercept_kappa+get(kappa_name) +
                                       b_kappa_CLu +get(kappac_name)))'
  ...
  Draws2Cont(draws_uni,
             x_string = 'sin(b_mu0_Intercept + get(mu_name) + 
                                   b_mu0_BRl + get(mub_name))*
                         A1(softplus(Intercept_kappa +get(kappa_name)+
                                           b_kappa_BRl +get(kappab_name)))',
             y_string = 'cos(b_mu0_Intercept + get(mu_name) + 
                                   b_mu0_BRl + get(mub_name))*
                         A1(softplus(Intercept_kappa+get(kappa_name)+
                                           b_kappa_BRl +get(kappab_name)))'
...
#condition specific
gh_ids = with(subset(cd_uni,
                     subset = CL %in% 'g' & 
                       BR %in% 'h'),
              unique(ID)
              )
uh_ids = with(subset(cd_uni,
                     subset = CL %in% 'u' & 
                       BR %in% 'h'),
              unique(ID)
              )
gl_ids = with(subset(cd_uni,
                     subset = CL %in% 'g' & 
                       BR %in% 'l'),
              unique(ID)
              )
ul_ids = with(subset(cd_uni,
                     subset = CL %in% 'u' & 
                       BR %in% 'l'),
              unique(ID)
              )
...
Draws2Cont(draws = draws_uni,
           x_string = 'sin(b_mu0_Intercept + b_mu0_CLu)*
             A1(kappa_muc)',
           y_string = 'cos(b_mu0_Intercept + b_mu0_CLu)*
             A1(kappa_muc)'#,
           # palette = 'Mint'
           )

with(draws_uni,
     arrows.circular(x = circular(x = MedCDraws(b_mu0_Intercept + b_mu0_CLu),
                                                  units = 'radians',
                                                  rotation = 'clock',
                                                  zero = pi/2),
...
Draws2Cont(draws = draws_uni,
           x_string = 'sin(b_mu0_Intercept + b_mu0_BRl)*
             A1(kappa_mub)',
           y_string = 'cos(b_mu0_Intercept + b_mu0_BRl)*
             A1(kappa_mub)'#,
           # palette = 'Mint'
           )

with(draws_uni,
     arrows.circular(x = circular(x = MedCDraws(b_mu0_Intercept + b_mu0_BRl),
                                                  units = 'radians',
                                                  rotation = 'clock',
                                                  zero = pi/2),
...
with(draws_uni,
     {
       round(deg(
     quantile(mod_circular(b_mu0_CLu), probs = c(0.5, 0.025, 0.975))
       ))
     }
)
...
with(draws_uni,
     {
       round(deg(
     quantile(mod_circular(b_mu0_BRl), probs = c(0.5, 0.025, 0.975))
       ))
     }
)
...
with(draws_uni,
     {
       round(
     quantile(softplus(Intercept_kappa + b_kappa_CLu) - 
...
with(draws_uni,
     {
       round(
     quantile(softplus(Intercept_kappa + b_kappa_BRl) - 
                softplus(Intercept_kappa),
              probs = c(0.5, 0.025, 0.975)),
     digits = 2
       )
...
#no mu colour priors
pr_nobr = pr_mu_nobr + pr_kappa_uni

#no mu brightness priors
pr_nocl = pr_mu_nocl + pr_kappa_uni
