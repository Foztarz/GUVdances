...
plot(x = 1:dim(lc_plot)[1],
     ...
     pch = 19,
     col = c(col_obs, col_kappa, col_treat, col_sd2, col_rho),
     cex = 2,
     axes = FALSE)
with(lc_plot,
     {
...
axis(4,
     at = with(lc_plot,
               {
               seq(from = elpd[3], to = elpd[dim(lc_plot)[1]] + se[dim(lc_plot)[1]]*4, by = 20)
               }
               ),
     labels = with(lc_plot,
                          {
                            seq(from = 0, to =  elpd[dim(lc_plot)[1]] + se[dim(lc_plot)[1]]*4 - elpd[3],  by = 20)
                          }
     ),
     col = col_rho
)
abline(h = lc_plot$elpd[3],
       col = 'gray')
